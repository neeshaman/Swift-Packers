...
<body class="cbp-spmenu-push">
	<div class="main-content">
				...
				<div class="tables">
					...
					<div class="bs-example widget-shadow" data-example-id="hoverable-table"> 
						<table class="table table-hover">
							...
							%>
								<tr>
									<th scope="row"><%=bean.getUserId()%></th>
		...
		try {
			...
			String selectQ = "select * from user_type where user_type_id="+bean.getUserTypeId();
			String Q="select * from security_question where security_question_id="+bean.getSecurity_questionId();
			System.out.println(selectQ+"\n"+Q);
			rs = stmt.executeQuery(selectQ);
			rs1=stmt1.executeQuery(Q);
		} 
		catch (Exception e) {
			System.out.println("Error ListUser.jsp::in select user_type && securityQuestion");
			e.printStackTrace();
		}
	%>									
								...
								<%if(rs.next()){ %>	
									<td><%=rs.getString("user_type_name")%></td>
								<%}
								if(rs1.next()){%>
									<td><%=rs1.getString("security_question")%></td>
								<%}%>
									<td><%=bean.getAnswer() %></td>
									<td><a href="EditUserServlet?userId=<%=bean.getUserId()%>">Edit</a> | <a
										href="DeleteUserServlet?userId=<%=bean.getUserId()%>">Delete</a></td>
								</tr>
							<%
							}
							%>
