...
<!--//Metis Menu -->
	...
	<%
		ArrayList<Vendor_detailBean> list = (ArrayList<Vendor_detailBean>) request.getAttribute("Vendor_detailList");
...
<body class="cbp-spmenu-push">
	<div class="main-content">
		...
		<div id="page-wrapper">
			<div class="main-page">
				<div class="tables">
					<h3 class="title1">Vendor_detail Table:</h3>
					<div class="bs-example widget-shadow" data-example-id="hoverable-table">
						<table class="table table-hover">
							<thead> 
								<tr>
									<th>Vendor_detail Id</th>
									<th>User</th>
									<th>Address</th>
									<th>MaxVehicle</th>
									<th>License</th>
		...
		try {
			...
			String selectQ = "select * from public.user where user_id = " + bean.getUserId();
			System.out.println(selectQ);
			rs = stmt.executeQuery(selectQ);
		} 
		catch (Exception e) {
			System.out.println("Error ListVendor_detail.jsp::");
			e.printStackTrace();
		}
	%>									
								...
								<td><%=bean.getMaxVehicle()%></td>
								<td><%=bean.getLicense()%></td>
								<td><%=bean.getCompanyName() %></td>
								<td><a href="EditVendor_detailServlet?vendor_detailId=<%=bean.getVendorDetailId()%>">Edit</a> | <a
									href="DeleteVendor_detailServlet?vendor_detailId=<%=bean.getVendorDetailId()%>">Delete</a></td>
							</tr>
					<%
						}	
					%>
