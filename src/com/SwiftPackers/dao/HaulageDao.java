...
public class HaulageDao {
	...
	public ArrayList<HaulageBean> getVenQuotationRequestHaulage() {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String selectQ = "select public.user.user_name , haulage.haul_id from public.user , haulage where public.user.user_id = haulage.cust_id and haulage.haul_status ='estimate'";
			System.out.println(selectQ);
			rs = stmt.executeQuery(selectQ);
			HaulageBean bean = null;
			while (rs.next())
