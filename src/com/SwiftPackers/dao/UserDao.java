...
public class UserDao {
	...
	public UserBean getDataByEmail(String emailId) {
		...
		try {
			...
			String selectQ = "select * from public.user where user_email_id = '" + emailId + "'";
			System.out.println(selectQ);
			ResultSet rs = stmt.executeQuery(selectQ);
			
			String Q = "SELECT * FROM public.user WHERE user_email_id = ?";
			PreparedStatement preparedStatement = con.prepareStatement(Q);
			preparedStatement.setInt(1, Integer.parseInt(emailId));
			...
			if (rs.next()) {
				...
				userBean.setSecurity_questionId(rs.getString("security_question_Id"));
				userBean.setAnswer(rs.getString("answer"));
				int securityQuestionId = rs.getInt("security_questionId");
				String selectS = "select * from public.security_question where security_question_id = " + securityQuestionId;
	...
	public UserBean authenticateUser(String email, String password) {
		...
		try {
			...
			String selectQ = "select * from public.user where user_email_id ='" + email + "' and user_password='" + password + "'";
			System.out.println(selectQ);
			ResultSet rs = stmt.executeQuery(selectQ);
	...
	public UserBean insertUser(UserBean bean) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String insertQ = "insert into public.user (User_type_id,User_name,User_email_id,User_phone_no,User_password,security_question_id,answer) values ("
			...
			if (i == 1)
			{
				...
				flag = true;
				ResultSet rs1 = stmt1.executeQuery("select user_id from public.user where user_email_id  = '"+bean.getUserEmail()+"'");
				rs1.next();
	...
	public UserBean insertGoogleUser(UserBean bean) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String insertQ = "insert into public.user (User_type_id,User_name,User_email_id,User_phone_no,User_password,security_questionId,answer) values (1,'" + bean.getUserName() + "','" + bean.getUserEmail() + "','1234567890','1234568',1,'12345678')";
			System.out.println(insertQ);
			int i = stmt.executeUpdate(insertQ);
			if (i == 1)
			{
				...
				Statement stmt1 = con1.createStatement();
				flag = true;
				ResultSet rs1 = stmt1.executeQuery("select * from public.user where user_email_id  = '"+bean.getUserEmail()+"'");
				rs1.next();
				String ss = rs1.getString("user_id");
				...
				bean.setSecurity_questionId(rs1.getString("security_question_Id"));
	...
	public boolean deleteUser(String userId) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String deleteQ = "delete from public.user where user_id = " + userId;
	...
	public UserBean getDataByPk(String userId) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String selectQ = "select * from public.user where user_id = " + userId;
			System.out.println(selectQ);
			rs = stmt.executeQuery(selectQ);
			if (rs.next()) {
				bean = new UserBean();
		...
		System.out.println("USER BEAN FROM DAO  -  " + bean);
		return bean;
	}
	...
	public boolean updateUser(UserBean bean) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String updateQ = "update public.user set user_name='" + bean.getUserName() + "' , user_email_id='"
					+ bean.getUserEmail() + "' , user_phone_no = '" + bean.getPhoneNo() + "' , user_type_id = "
					+ bean.getUserTypeId() + ",security_question_id= " + bean.getSecurity_questionId() + " ,answer ='"
	...
	public boolean updatePassword(String userId, String password) {
		...
		try {
			con = DbConnection.getConnection();
			stmt = con.createStatement();
			String updateQ = "update public.user set user_password='" + password + "' where user_id = " + userId;
			System.out.println(updateQ);
			int i = stmt.executeUpdate(updateQ);
			if (i == 1) {
				flag = true;
